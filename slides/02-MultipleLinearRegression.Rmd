...
- Testing the **individual** regression coefficients for $H_0: \beta_j = \beta_j^0$ versus $H_1: \beta_j \neq \beta_j^0$ can be done using the t-test by comapring the observed value $t_j$ with the appropriate critical value $t_{n-p-1,\alpha/2}$.
...
- An equivalent decision can be made using the p-value and reject $H_0$ if $p(|t_j|) \leq \alpha$. The p-value is the probabillity that a random variable hacinf a Student t-distribution with n-p-1 degrees of freedoms is greater than $|t_j|$ (the observed value of the t-test).
