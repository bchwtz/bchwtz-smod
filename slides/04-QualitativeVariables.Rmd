...
\begin{alertblock}{}
\tiny
When different models are required for the groups, this would imply that the required score in the preemployment test that is needed to result in (minimum) job performance (horizontal dashed line) needs to distinguished by group (vertical dashed lines).
\end{alertblock}
\end{textblock}

## Models with different Slopes and different Intercepts

- What we want to test for the Preemployment Test data is for differences in intercept and slope using the following Null.
...
- Model 1 can be seen as a restriced verison (RM) of model 3,  the full model (FM), with $\gamma=\delta=0$.
