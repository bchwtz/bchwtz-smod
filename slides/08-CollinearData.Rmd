...
- A thorough investigation of muticollinearity will involve examining the value of $R^2$ that restulst from regression **each of the predictors against all others**.
- The resulting effects can be judged by examining a quantitiy called *cariance inflation index* (**VIF**).
...
- The condition indices are based on the eigenvalues $\lambda_1, \lambda_2, \ldots, \lambda_p$ of them correlation matrix. If any $\lambda_ =0$, there is perfect linear relationship, which is an extreme case of collinearity. Strong heterogeneity in the eigenvalues (one value much smaller than the others) also indicates mulitcollinearity.
- An empirical criterion for the presence of collinearity is given by the sum of the recoprocals of the eigenvalues of the correlation matrix. If that sum is much larger (e.g. 5 times larger) than the number of predictor variables $p$, collinearity is present. 
...
- Usually one wants to know **which variables are involved in the collinearity problem**.
- This can be answered using the **eigen vectors** of the correlation matrix, which are pairwise orthogonal.
- We examine this using the import dataset:

```{r}
cormat <- cor(P241[c("DOPROD", "STOCK", "CONSUM")])
eigen(cormat)
```
