...
```
The object `q` contains three variables (columns), the question ID `Q`, an indication if the question is positively (`coding == "P"`) or negatively (`coding == "N"`)  formulated and the actaul formulation of the item `question`. The responses are located in the object `d`.
...
In total we have `r nrow(d)` observations and `r ncol(d)` 
